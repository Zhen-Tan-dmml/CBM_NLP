...
def mixup_hidden_concept(h, c, y, alpha=0.2):
    # print(h.size()[0])
    # print(c.size()[0])
    # print(y.size()[0])
...
class MixupLoss(torch.nn.Module):
    ...
    def forward(self, output, target, lam):
        assert output.size()[0] // 2 == target.size(0) // 3

        orginal_len = target.size(0) // 3

        original_loss = self.ce_loss(output[: orginal_len], target[: orginal_len])
        # loss = lam * F.cross_entropy(output, target_a) + (1 - lam) * F.cross_entropy(output, target_b)
        mixed_loss = lam * self.ce_loss(output[orginal_len:], target[orginal_len: 2*orginal_len]) + (1 - lam) * self.ce_loss(output[orginal_len:], target[orginal_len: 2*orginal_len])
        loss = torch.cat([original_loss, mixed_loss]).mean()
        return loss
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader, desc="Training", unit="batch"):
        ...
        mixed_h, c_a, c_b, y_a, y_b, lam = mixup_hidden_concept(pooled_output, concept_labels, label, alpha=0.2)

        all_c = torch.cat([concept_labels, c_a, c_b], dim=0)
        all_c = torch.t(all_c)
        all_c = all_c.contiguous().view(-1) 
        # c_b = torch.t(c_b)
        # c_b = c_b.contiguous().view(-1) 
        # concept_labels = torch.t(all_c)
        # concept_labels = concept_labels.contiguous().view(-1) 

        all_y = torch.cat([label, y_a, y_b], dim=0)

        all_h = torch.cat((pooled_output, mixed_h))
        ...
        XtoC_loss = loss_fn(XtoC_logits, all_c, lam)
        # XtoY_loss
        # XtoY_loss = loss_fn(XtoY_output[0], label)
        XtoY_loss = loss_fn(XtoY_output[0], all_y, lam)
